...
type AccountHandler struct {
	...
	northWindService services.NorthWindServiceInterface
}

// NewAccountHandler creates a new account handler
func NewAccountHandler(accountService services.AccountServiceInterface, auditLogger services.AuditLoggerInterface, metricsCollector services.MetricsRecorderInterface, northWindService services.NorthWindServiceInterface) *AccountHandler {
...
func (h *AccountHandler) Transfer(c echo.Context) error {
	...
	transfer, err := h.accountService.TransferBetweenAccounts(fromAccountID, toAccountID, amount, req.Description, idempotencyKey, req.Currency, req.TransferType, req.Direction, userID)
...
func (h *AccountHandler) GetTransferHistory(c echo.Context) error {
	...
	filters := models.TransferFilters{
		Status: models.NormalizeTransferStatus(c.QueryParam("status")),
	}

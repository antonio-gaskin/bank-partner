...
// FindPendingTransfers finds all pending transfers
func (r *transferRepository) FindPendingTransfers() ([]*models.Transfer, error) {
	var transfers []*models.Transfer

	if err := r.db.Where("status = ?", models.TransferStatusPending).
		Order("created_at ASC").
		Find(&transfers).Error; err != nil {
		return nil, fmt.Errorf("failed to find pending transfers: %w", err)
	}

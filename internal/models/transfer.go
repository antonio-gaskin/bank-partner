...
type Transfer struct {
	...
	TransferId          string          `gorm:"type:uuid;not null" json:"transfer_id"`
	ReferenceNumber     string          `gorm:"type:uuid;not null" json:"reference_number"`
	TransferType        string          `gorm:"type:uuid;not null" json:"transfer_type"`
...
func (t *Transfer) Complete(debitTxID, creditTxID uuid.UUID, status string) {
	t.Status = status
	now := time.Now()
	t.CompletedAt = &now
...
// NormalizeTransferStatus converts a status string to the canonical uppercase constant

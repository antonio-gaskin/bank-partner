...
func (s *accountService) TransferBetweenAccounts(
	...
	description, idempotencyKey, currency, transferType, direction string,
	userID uuid.UUID,
	...
	sourceUser, err := s.userRepo.GetByID(fromAccount.UserID)
	if err != nil {
		return nil, err
	}
	destinationUser, err := s.userRepo.GetByID(toAccount.UserID)
	if err != nil {
		return nil, err
	}
	referenceNumber := models.GenerateTransactionReference()

	validationRes, err := s.northWindService.ValidateTransfer(
		referenceNumber,
	...
	if err != nil {
		s.logger.Error("Validate Transfer ERROR", "error", err)
		return nil, err
	}
	s.logger.Info("Validation Response", "validationRes", validationRes)

	if !validationRes.Validation.Valid {
		return nil, errors.New("transfer validation failed with external service")
	}

	initiated, err := s.initiateTransfer(
		referenceNumber,
		sourceUser, destinationUser,
		fromAccount, toAccount,
		amount,
	...
	transfer, debitTxID, creditTxID, err := s.executeTransfer(
		...
		fromAccount, toAccount, initiated.ReferenceNumber, initiated.Status, initiated.TransferID, initiated.TransferType,
	)
	if err != nil {
		if transfer != nil {
			s.handleTransferFailure(transfer, err, fromAccount, toAccount, amount, idempotencyKey, userID, initiated.Status)
		}
		return nil, err
	}

	if err := s.handleTransferSuccess(transfer, debitTxID, creditTxID, fromAccount, toAccount, amount, idempotencyKey, userID, models.TransferStatusCompleted); err != nil {
		return nil, err
	}
...
func (s *accountService) initiateTransfer(
	...
	initiateReq := dto.NorthWindInitiateTransferRequest{
		Amount:             amountFloat,
		Currency:           currency,
		Description:        description,
	...
	// s.logger.Info("request data", "initiateReq", initiateReq)

	res, err := s.northWindService.InitiateTransfer(ctx, initiateReq)
	if err != nil {
...
func (s *accountService) executeTransfer(
	...
	fromAccount, toAccount *models.Account,
	referenceNumber, status, transferId, transferType string,
...
func (s *accountService) handleTransferSuccess(
	...
	transfer.Complete(debitTxID, creditTxID, status)
	if err := s.transferRepo.Update(transfer); err != nil {

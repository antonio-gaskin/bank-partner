...
func (s *TransferServiceTestSuite) TestTransferBetweenAccounts_Success() {
	...
	// Mock user lookups
	sourceUser := &models.User{ID: userID, Email: "source@example.com", FirstName: "Source", LastName: "User"}
	destUser := &models.User{ID: toAccount.UserID, Email: "dest@example.com", FirstName: "Dest", LastName: "User"}
	s.userRepo.EXPECT().GetByID(userID).Return(sourceUser, nil)
	s.userRepo.EXPECT().GetByID(toAccount.UserID).Return(destUser, nil)

	// Mock NorthWind validation
	s.northWindService.EXPECT().
		ValidateTransfer(gomock.Any(), sourceUser, destUser, fromAccount, toAccount, amount, "USD", "internal", "Test transfer", idempotencyKey, "outbound").

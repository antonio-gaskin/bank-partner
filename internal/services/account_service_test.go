...
type AccountServiceSuite struct {
	...
	northWindService *service_mocks.MockNorthWindServiceInterface
...
func (s *AccountServiceSuite) SetupTest() {
	...
	s.northWindService = service_mocks.NewMockNorthWindServiceInterface(s.ctrl)
	s.service = NewAccountService(s.accountRepo,
		...
		s.auditRepo,
		slog.Default(),
		s.northWindService).(*accountService)
...
func (s *AccountServiceSuite) TestCreateAccount_WithInitialDeposit() {
	...
	account, err := s.service.CreateAccount(s.testUserID, "checking", "1012345678", "123456789", decimal.NewFromFloat(100))
	s.NoError(err)
	s.NotNil(account)
	s.Equal(s.testUserID, account.UserID)
	s.Equal("checking", account.AccountType)
	s.Equal("1012345678", account.AccountNumber)
	s.Equal("123456789", account.RoutingNumber)
...
func (s *AccountServiceSuite) TestCreateAccount_WithoutInitialDeposit() {
	...
	account, err := s.service.CreateAccount(s.testUserID, "savings", "2012345679", "987654321", decimal.Zero)
	s.NoError(err)
	s.NotNil(account)
...
func (s *AccountServiceSuite) TestCreateAccount_UserNotFound() {
	...
	account, err := s.service.CreateAccount(s.testUserID, "checking", "dummy", "dummy", decimal.Zero)
	s.Error(err)
	s.Nil(account)
...
func (s *AccountServiceSuite) TestCreateAccount_NegativeInitialDeposit() {
	...
	account, err := s.service.CreateAccount(s.testUserID, "checking", "dummy", "dummy", decimal.NewFromFloat(-100))
	s.Error(err)
	s.Nil(account)
...
func (s *AccountServiceSuite) TestTransferBetweenAccounts_Success() {
	...
	s.userRepo.EXPECT().GetByID(s.testUserID).Return(s.testUser, nil).Times(2)

	// Validate transfer with NorthWind
	s.northWindService.EXPECT().
		ValidateTransfer(
			gomock.Any(),     // referenceNumber
			s.testUser,       // sourceUser
			s.testUser,       // destinationUser
			fromAccount,      // fromAccount
			toAccount,        // toAccount
			amount,           // amount
			"USD",            // currency
			"internal",       // transferType
			"Transfer funds", // description
			idempotencyKey,   // idempotencyKey
			"outbound",       // direction
		).
		Return(&dto.NorthWindValidateResponse[dto.NorthWindTransferStatusResponse]{
			Validation: dto.NorthWindValidation{Valid: true},
			Data:       &dto.NorthWindTransferStatusResponse{Status: "approved"},
		}, nil)

	// Initiate transfer with NorthWind
	s.northWindService.EXPECT().
		InitiateTransfer(gomock.Any(), gomock.Any()).
		Return(&dto.NorthWindTransferStatusResponse{Status: "initiated"}, nil)
	...
	_, err := s.service.TransferBetweenAccounts(fromAccountID, toAccountID, amount, "Transfer funds", idempotencyKey, "USD", "internal", "outbound", s.testUserID)
	s.NoError(err)
}
...
func (s *AccountServiceSuite) TestTransferBetweenAccounts_SameAccount() {
	...
	_, err := s.service.TransferBetweenAccounts(accountID, accountID, decimal.NewFromFloat(100), "placeholder-description", "placeholder-idempotency-key", "USD", "internal", "outbound", s.testUserID)
	s.Error(err)
...
func (s *AccountServiceSuite) TestTransferBetweenAccounts_InvalidAmount() {
	...
	_, err := s.service.TransferBetweenAccounts(fromAccountID, toAccountID, decimal.NewFromFloat(-100), "placeholder-description", "placeholder-idempotency-key", "USD", "internal", "outbound", s.testUserID)
	s.Error(err)
	s.Equal(ErrInvalidAmount, err)
}
